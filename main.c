...
void frame_copy(struct frame_t *frame, struct picture_t *pic)
{
		...
		if (pkt.stream_index == video_stream && parse_mpeg(&mpeg))
		{
			...
			if (frames[disp_frame % RING_BUFFER_SIZE] != NULL)
			{
				...
				if (firsttime) {
					...
					pic.timestamp.tv_sec = disp_frame / 25;
					pic.timestamp.tv_usec = (disp_frame * 40000) % 1000000;	// 25 FPS
